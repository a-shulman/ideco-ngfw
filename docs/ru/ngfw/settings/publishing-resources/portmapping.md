...
**Не рекомендуется использовать проброс портов для публикации веб- и почтовых серверов (80, 443 порты).** Для их публикации воспользуйтесь [обратным прокси-сервером](../../../ngfw/settings/services/reverse-proxy.md).

{% endnote %}

{#top}

{% cut "Особенности создания правил DNAT в версии 18" %}

* В предыдущих версиях профили безопасности настраивались в отдельных разделах. В версии 18 **[Контроль приложений](../../../ngfw/settings/security-profiles/application-control/README.md)** и система **[Предотвращения вторжений](../../../ngfw/settings/security-profiles/ips-profiles/README.md)** настраиваются в правилах **Файрвола**;
...
* В случае проблем с доступом до публикуемой службы проверьте **Отчеты и журналы -> События безопасности -> Журнал IPS** и при необходимости отредактируйте правила нужного профиля **[Предотвращения вторжений](../../../ngfw/settings/security-profiles/ips-profiles/README.md)**.

{% endcut %}

{#top}

{% cut "Создание правил DNAT и FORWARD в Файрволе Ideco NGFW" %}
...
* Если в одной локальной сети находятся пользователи и сервер с опубликованным при помощи DNAT-правила ресурсом, вероятна асимметричная маршрутизация. Информация о способах устранения асимметричной маршрутизации трафика представлена в [статье](../../../ngfw/recipes/popular-recipes/access-to-remote-networks.md).

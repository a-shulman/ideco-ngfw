...
![](../../../_images/balancer.png)

Подключение, которое используется в данный момент, отмечено тегом **Используется**. Для смены приоритета используйте соответствующие элементы управления (![up-down.png](../../../_images/icon-up-down.png)).
...
![](../../../_images/balancer2.png)

Сервер посылает на эти адреса ping-запросы. Соединение с интернетом считается установленным, если проходит пинг хотя бы до одного адреса из списка. Если этого не происходит, NGFW считает, что соединение с интернетом у интерфейса отсутствует - статус интерфейса меняется с ![](../../../_images/icon-internet-on.png) на ![](../../../_images/icon-internet-off.png).

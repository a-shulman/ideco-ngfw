...
Порядок обработки веб-трафика подробнее описан в [статье](../../../../ngfw/recipes/popular-recipes/processing-order.md).
...
Для настройки фильтрации HTTPS-трафика нужно добавить корневой сертификат NGFW на компьютеры пользователей. Подробнее в статье [Настройка фильтрации HTTPS](../../../../ngfw/settings/access-rules/content-filter/filtering-https-traffic.md).
...
О настройке прямого подключения к прокси и прокси с одним интерфейсом читайте в [статье](../../../../ngfw/settings/services/proxy/proxy-single-interface.md).
...
В NGFW предусмотрено два режима работы WCCP - L2 и GRE. Для выбора режима раскройте блок **Настройки**. Для работы WCCP в GRE-режиме необходимо также создать на Ideco NGFW [GRE-интерфейс](../../../../ngfw/settings/services/connection-to-provider/README.md#туннельные-интерфейсы), указав в поле **Адрес удаленного интерфейса** IP-адрес WCCP-сервера.
...
Подробнее о типах исключений в статье [Исключения](../../../../ngfw/settings/services/proxy/exclusions.md).
